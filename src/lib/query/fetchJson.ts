...
// carrying the status code the retry predicate narrows on (queryClient.ts).
//
...
// client boundary instead, e.g. order-book/binanceSchemas.ts).
// Richer clients that need error bodies subclass HttpError so the `status < 500` retry
// check keeps working on their errors — BinanceHttpError (order-book/binanceRest.ts) is
// the live example.
...
// it (queryClient.ts). `cause` preserves the original SyntaxError for the reportError seam
